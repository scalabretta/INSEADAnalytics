...
**Your Answers here:**
<1>in my tickers it has a vector of SPY and AAPL, so all operations are done on both the stock index and the apple stock. 
<2>cumulative returns are 596%
average daily returns are 0.148%
st dev of returns is 2.39%
<3>all you need to do is change the mytickers vector in dataSet1.R to the ticker of Yahho and run the code again, the apply the above formulas.
291%, 0.072%, 3.02%
<br>
...
**Your Answers here:**
<1> either dim, or nrow and ncolumns
<2> 
> portfolio = apply(StockReturns,1,mean)
> names(portfolio) <- rownames(StockReturns)
> pnl_plot(portfolio)
...
**Your Answers here:**
<1> need to change the definition of the start date variable in the code. (from whatever it is previously to 2001-01-01)
<2>  all you need to do is add tickers in the ticker variabe definition.
...
**Your Answers here:**
<1> you get an answer of 20. the command should be summing when myData is not equal to stock returns so i would expect to get zero. Might have to do with the fact that there is a column name in myData.
<2> The command does not work- maybe becayse MyData is now a list not a vector necessarily so summing it with the vector does not work- the answer is that it is a non numerica argument. 
<br>
<br>
